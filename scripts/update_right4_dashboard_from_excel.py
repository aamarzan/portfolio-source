...
def load_master_dataframe(workbook_path: Path) -> pd.DataFrame:
    ...
    df["Date of Screening"] = pd.to_datetime(df["Date of Screening"], dayfirst=True, errors="coerce")
...
def build_payload(df: pd.DataFrame) -> dict:
    ...
    payload = {
        "meta": {
            ...
            "latestScreeningDate": df["Date of Screening"].max().date().isoformat() if len(df) else "",
        },
        "summary": summary,
        "siteTables": {
