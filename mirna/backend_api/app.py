# app.py (fully updated, consolidated ~500+ lines)
...
def choose_mature_window(seq: str, window: int = 22) -> str:
    ...
    if len(s) <= window:
        return s
    # simple heuristic: balanced GC and minimal homopolymers
    import itertools
    def score(sub: str):
...
def parse_fasta_records(text: str):
    ...
    try:
        ...
        # Fallback raw parse: naive, expects >id headers if present
...
def validate_structure_matches_sequence(struct_kind: Optional[str], struct_seq: str,
    ...
    try:
        ...
        # If Biopython align is missing, skip validation but warn
...
def start_prediction():
    ...
    mirna_3d_index: Dict[str, Tuple[Optional[str], str]] = {}
        ...
        for start in range(0, len(primary_records), BATCH_SIZE):
            ...
            for pri_id, pri_seq in batch_records:
                ...
                if pri_id in mirna_3d_index:
                    kind, seq3d = mirna_3d_index[pri_id]
