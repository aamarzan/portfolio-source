...
function injectResultFilters(){ if($('result-filters')) return; const box=document.createElement('div'); box.id='result-filters'; box.className='result-filters'; box.style.cssText='display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:8px 0 12px;justify-content:center;'; box.innerHTML=`
  ...
  `; byQS('#results-container')?.insertBefore(box,byQS('#results-container').firstChild); const apply=()=>{ const rOnly=byQS('#filter-range')?.checked||false; const tOnly=byQS('#filter-tolerant')?.checked||false; $('chip-range')?.classList.toggle('active',rOnly); $('chip-tol')?.classList.toggle('active',tOnly); byQSA('#results-table tbody tr').forEach(tr=>{ const hasRange=tr.getAttribute('data-range')==='1'; const isTol=tr.getAttribute('data-tolerant')==='1'; const hide=(rOnly&&!hasRange)||(tOnly&&!isTol); tr.style.display=hide?'none':''; }); }; bindOnce($('filter-range'),'change',apply,'rFilter'); bindOnce($('filter-tolerant'),'change',apply,'tFilter'); bindOnce($('clear-filters'),'click',()=>{ const r=$('filter-range'),t=$('filter-tolerant'); if(r) r.checked=false; if(t) t.checked=false; apply(); },'cFilter'); }

// =====================================================
// Wire existing analysis controls (HTML block) once
// =====================================================
function injectAnalysisControls(container){ if(GUARDS.analysisControlsInjected) return; const html=`
  ...
  `; prependHTML(container,html); const seedBtn=$('seed-scan-global-btn'),hmBtn=$('explain-global-btn'); bindOnce(seedBtn,'click',async()=>{ if(!predictionResults.length){ openModal('Seed Sites',formatInfo('Run a prediction first so we can use the top-ranked row.')); return; } await handleSeedSitesClick(predictionResults[0]); },'seedGlobalOnce'); bindOnce(hmBtn,'click',async()=>{ if(!predictionResults.length){ openModal('Heatmap',formatInfo('Run a prediction first so we can use the top-ranked row.')); return; } await handleHeatmapClick(predictionResults[0]); },'hmGlobalOnce'); GUARDS.analysisControlsInjected=true; }

// =====================================================
// CSV & bundles
// =====================================================
async function handleRowCsvClick(item){ if(!CURRENT_JOB_ID){ alert('No active job.'); return; } const interactionId=item.interaction_id||null; if(!interactionId){ alert('Row is missing interaction_id.'); return; } try{ const headers=await getNonceOrKeyHeaders(); const res=await fetch(DOWNLOAD_ROW_CSV_URL(CURRENT_JOB_ID,interactionId),{method:'GET',headers}); if(!res.ok) throw new Error('Download failed'); const blob=await res.blob(); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`interaction_${interactionId}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }catch(err){ alert('Could not download row CSV.'); } }
async function handleBundleClick(item){ if(!CURRENT_JOB_ID){ alert('No active job.'); return; } const interactionId=item.interaction_id||null; if(!interactionId){ alert('Row is missing interaction_id.'); return; } try{ const headers=await getNonceOrKeyHeaders(); const res=await fetch(`${BASE_URL}/download/${CURRENT_JOB_ID}/${interactionId}/bundle.zip`,{method:'GET',headers}); if(!res.ok) throw new Error('Download failed'); const blob=await res.blob(); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`interaction_${interactionId}.zip`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }catch(err){ alert('Could not download this interaction bundle.'); } }

// =====================================================
// Heatmap (server PNG → retry; then /explain → client PNG;
// plus pure client modes: seed_density & seed_matrix)
// =====================================================
async function handleHeatmapClick(item){ const modeSel=byQS('#heatmap-mode'),stepsInp=byQS('#heatmap-steps'); const rawMode=(modeSel?.value||'ig_target').toLowerCase(); const steps=Math.max(10,Math.min(200,parseInt(stepsInp?.value||'64',10)||64)); openModal('Heatmap',smallSpinner('Generating heatmap...')); const effMode=(rawMode==='ig_competitor'&&!(item.competitor_id||'').trim())?'ig_target':rawMode; const isPureClient=effMode==='seed_density'; if(!isPureClient&&CURRENT_JOB_ID&&item.interaction_id){ try{ const headers=await getNonceOrKeyHeaders(); const res=await fetchRetry(HEATMAP_PNG_URL(CURRENT_JOB_ID,item.interaction_id,effMode,steps),{method:'GET',headers},45000,2,700); if(res.ok){ const blob=await res.blob(); const url=URL.createObjectURL(blob); const html=`
          <div class="hm-wrapper">
            <div class="hm-toolbar">
              <button class="btn btn-sm btn-outline" id="hm-download-btn">Download PNG</button>
            </div>
            <img id="hm-img" src="${url}" alt="Heatmap" style="max-width:100%;height:auto;border-radius:8px;border:1px solid #e5e7eb;"/>
          </div>
        `; openModal('Heatmap',html); setTimeout(()=>{ const btn=document.getElementById('hm-download-btn'); if(btn){ btn.addEventListener('click',()=>{ const a=document.createElement('a'); a.href=url; a.download=`${item.interaction_id||'heatmap'}.png`; a.click(); }); } },0); return; } }catch(err){ console.warn('Server heatmap PNG failed, falling back to client IG/seed.',err); } } await clientExplainHeatmapFallback(item,effMode,true); }

async function clientExplainHeatmapFallback(item,forcedMode,forceCanvasPNG=false){ const mirnaId=item.primary_molecule_id??item.mirna_id; const targetId=item.target_id??''; const compId=item.competitor_id??''; const mirnaSeqRaw=lookupTolerant(CURRENT_INPUTS.mirnas,mirnaId); const tRes=resolveSeqWithAAHandling(targetId,CURRENT_INPUTS.targets); const cRes=compId?resolveSeqWithAAHandling(compId,CURRENT_INPUTS.competitors):{seq:'',converted:false,note:'',mode:''}; const targetSeq=tRes.seq,compSeq=cRes.seq,mirnaSeq=mirnaSeqRaw?toRNA(mirnaSeqRaw):''; if(!mirnaSeq||!targetSeq){ const mc=$('modal-content'); if(mc) setHTML(mc,formatError('Could not resolve miRNA and/or target sequences for this row.')); return; } const uiMode=(forcedMode||byQS('#heatmap-mode')?.value||'ig_target').toLowerCase(); const uiSteps=Math.max(10,Math.min(200,parseInt(byQS('#heatmap-steps')?.value||'64',10)||64)); // 1) PURE CLIENT seed_density
  if(uiMode==='seed_density'){ const density=computeSeedDensityArray(targetId,targetSeq,mirnaSeq); const vals=normalizeArray(density); const canvas=makeHeatCanvas(targetSeq,vals,`Seed density — ${targetId}`); showCanvasAsModalPNG(canvas,'Heatmap — Seed density',`${(item.interaction_id||'local')}_seed_density.png`); if(tRes.converted||cRes.converted){ const modeTxt=byQS('#aa-nt-mode')?.value||'canonical'; const mc=$('modal-content'); if(mc){ appendHTML(mc,`<div style="margin-top:6px;color:#333;"><small><em>AA→NT conversion applied (${escapeHTML(modeTxt)}) before visualization.</em></small></div>`); } } return; }
  // (duplicate seed_density block kept exactly per original)
  if(uiMode==='seed_density'){ const density=computeSeedDensityArray(targetId,targetSeq,mirnaSeq); const vals=normalizeArray(density); const canvas=makeHeatCanvas(targetSeq,vals,`Seed density — ${targetId}`); showCanvasAsModalPNG(canvas,'Heatmap — Seed density',`${(item.interaction_id||'local')}_seed_density.png`); if(tRes.converted||cRes.converted){ const modeTxt=byQS('#aa-nt-mode')?.value||'canonical'; const mc=$('modal-content'); if(mc){ appendHTML(mc,`<div style="margin-top:6px;color:#333;"><small><em>AA→NT conversion applied (${escapeHTML(modeTxt)}) before visualization.</em></small></div>`); } } return; }
  try{ const mc=$('modal-content'); if(mc) setHTML(mc,smallSpinner('Computing attributions.')); const headers=await getNonceOrKeyHeaders(); const body=JSON.stringify({mirna_seq:mirnaSeq,target_seq:targetSeq,competitor_seq:compSeq||undefined,steps:uiSteps,mode:uiMode}); let data=null; try{ const res=await fetchRetry(EXPLAIN_URL,{method:'POST',headers:{'Content-Type':'application/json',...headers},body},45000,1,700); data=res.ok?await res.json():null; }catch(_){ data=null; } if(data&&(Array.isArray(data.target_attrib)||Array.isArray(data.attribution))){ const targAttr=Array.isArray(data.target_attrib)?data.target_attrib:data.attribution; const compAttr=Array.isArray(data.competitor_attrib)?data.competitor_attrib:null; const targVals=normalizeArray(targAttr.slice(0,targetSeq.length)); const titleT=`IG → Target — ${targetId}`; const cnvT=makeHeatCanvas(targetSeq,targVals,titleT); if(uiMode==='ig_competitor'&&compSeq&&Array.isArray(compAttr)){ const cVals=normalizeArray(compAttr.slice(0,compSeq.length)); const titleC=`IG → Competitor — ${compId}`; const cnvC=makeHeatCanvas(compSeq,cVals,titleC); const stack=document.createElement('canvas'); const pad=12; const W=Math.max(cnvT.width,cnvC.width); const H=cnvT.height+cnvC.height+pad; stack.width=W; stack.height=H; const g=stack.getContext('2d'); g.fillStyle='#fff'; g.fillRect(0,0,W,H); g.drawImage(cnvT,0,0); g.drawImage(cnvC,0,cnvT.height+pad/2); showCanvasAsModalPNG(stack,'Heatmap — IG (target + competitor)',`${(item.interaction_id||'local')}_ig_both.png`); }else{ showCanvasAsModalPNG(cnvT,'Heatmap — IG → Target',`${(item.interaction_id||'local')}_ig_target.png`); } if(tRes.converted||cRes.converted){ const modeTxt=byQS('#aa-nt-mode')?.value||'canonical'; const mc2=$('modal-content'); if(mc2){ appendHTML(mc2,`<div style="margin-top:6px;color:#333;"><small><em>AA→NT conversion applied (${escapeHTML(modeTxt)}) for ${tRes.converted?'target':''}${tRes.converted&&cRes.converted?' & ':''}${cRes.converted?'competitor':''}.</em></small></div>`); } } return; }
    const density=computeSeedDensityArray(targetId,targetSeq,mirnaSeq); const vals=normalizeArray(density); const canvas=makeHeatCanvas(targetSeq,vals,`Seed density — ${targetId}`); showCanvasAsModalPNG(canvas,'Heatmap — Seed-based fallback',`${(item.interaction_id||'local')}_seed_profile.png`); const mc3=$('modal-content'); if(mc3){ appendHTML(mc3,`<div style="margin-top:6px;color:#333;"><small><em>Attribution service unavailable; showing a seed-based profile instead.</em></small></div>`); }
  }catch(err){ const density=computeSeedDensityArray(targetId,targetSeq,mirnaSeq); const vals=normalizeArray(density); const canvas=makeHeatCanvas(targetSeq,vals,`Seed density — ${targetId}`); showCanvasAsModalPNG(canvas,'Heatmap — Seed-based fallback',`${(item.interaction_id||'local')}_seed_profile.png`); const mc=$('modal-content'); if(mc){ appendHTML(mc,`<div style="margin-top:6px;color:#333;"><small><em>${escapeHTML(err?.message||'Attribution failed; showing a seed-based profile instead.')}</em></small></div>`); } } }

// Smooth Viridis-style colour map used by heatmaps
function viridisColor(value,alpha=1){ const palette=[[68,1,84],[59,82,139],[33,144,141],[93,201,99],[253,231,37]]; const v=Math.max(0,Math.min(1,Number.isFinite(value)?value:0)); const pos=v*(palette.length-1); const i0=Math.floor(pos); const i1=Math.min(i0+1,palette.length-1); const t=pos-i0; const r=Math.round(palette[i0][0]+t*(palette[i1][0]-palette[i0][0])); const g=Math.round(palette[i0][1]+t*(palette[i1][1]-palette[i0][1])); const b=Math.round(palette[i0][2]+t*(palette[i1][2]-palette[i0][2])); return `rgba(${r},${g},${b},${alpha})`; }

// 1D letter-strip canvas, normalization, seed density
function makeHeatCanvas(seq,values01,titleText){ const pad=14,bw=12,gap=2,bh=22; const W=pad*2+(bw+gap)*seq.length; const H=pad*2+bh+26; const c=document.createElement('canvas'); c.width=Math.max(200,W); c.height=H; const g=c.getContext('2d'); g.fillStyle='#ffffff'; g.fillRect(0,0,c.width,c.height); if(titleText){ g.fillStyle='#111'; g.font='600 14px ui-monospace, Menlo, Consolas, monospace'; g.textAlign='left'; g.textBaseline='top'; g.fillText(titleText,pad,6); } const y=pad+14; g.font='bold 12px ui-monospace, Menlo, Consolas, monospace'; g.textAlign='center'; g.textBaseline='middle'; for(let i=0;i<seq.length;i++){ const x=pad+i*(bw+gap); const v=Math.max(0,Math.min(1,values01[i]||0)); g.fillStyle=viridisColor(v,0.95); g.fillRect(x,y,bw,bh); g.fillStyle='#000'; g.fillText(seq[i]||'',x+bw/2,y+bh/2); } g.strokeStyle='#e5e7eb'; g.strokeRect(0.5,0.5,c.width-1,c.height-1); return c; }

function normalizeArray(arr){ if(!Array.isArray(arr)||!arr.length) return []; const max=arr.reduce((m,v)=>{ const a=Math.abs(v||0); return a>m?a:m; },1e-12); return arr.map(v=>Math.abs(v||0)/max); }

// Compute seed-density / seed-profile array
function computeSeedDensityArray(targetId,targetSeq,mirnaSeq){ const L=targetSeq.length||0; let density=new Array(L).fill(0); if(Array.isArray(LAST_SEED_HITS)&&LAST_SEED_HITS.length){ LAST_SEED_HITS.filter(h=>h.molecule==='target'&&h.id===targetId).forEach(h=>{ for(let i=Math.max(0,h.start-1);i<Math.min(L,h.end);i++) density[i]+=1; }); if(density.some(v=>v>0)) return density; } if(mirnaSeq) return computeSeedMatchProfile(mirnaSeq,targetSeq); return density; }

// Seed-match profile helpers
function normBase(b){ if(!b) return ''; b=b.toUpperCase(); return b==='T'?'U':b; }
function seedPairScore(mi,tg){ mi=normBase(mi); tg=normBase(tg); if((mi==='A'&&tg==='U')||(mi==='U'&&tg==='A')) return 1; if((mi==='C'&&tg==='G')||(mi==='G'&&tg==='C')) return 1; if((mi==='G'&&tg==='U')||(mi==='U'&&tg==='G')) return 0.5; return 0; }

function computeSeedMatchProfile(mirnaSeq,targetSeq){ if(!mirnaSeq||!targetSeq) return []; mirnaSeq=String(mirnaSeq).trim().toUpperCase(); targetSeq=String(targetSeq).trim().toUpperCase(); const Lm=mirnaSeq.length,Lt=targetSeq.length; if(Lm<7||Lt<7) return new Array(Lt).fill(0); const seedStart=1,seedLen=Math.min(7,Lm-seedStart),seed=mirnaSeq.slice(seedStart,seedStart+seedLen); const posScores=new Array(Lt).fill(0); for(let offset=0;offset<=Lt-seedLen;offset++){ let score=0; for(let s=0;s<seedLen;s++){ const mi=seed[seedLen-1-s]; const tg=targetSeq[offset+s]; score+=seedPairScore(mi,tg); } for(let s=0;s<seedLen;s++){ const idx=offset+s; if(score>posScores[idx]) posScores[idx]=score; } } return posScores; }

// Build premium seed-site summary chips
function buildSeedChipSummaryHTML(){ if(!Array.isArray(LAST_SEED_HITS)||!LAST_SEED_HITS.length) return ''; const groups=new Map(); for(const h of LAST_SEED_HITS){ const key=`${h.molecule}:${h.id}`; if(!groups.has(key)){ groups.set(key,{molecule:h.molecule,id:h.id,count:0,bestSeed:'',minMM:Infinity,wobbleHits:0}); } const g=groups.get(key); g.count++; if(!g.bestSeed&&h.seed_type) g.bestSeed=h.seed_type; const mm=typeof h.mismatches==='number'?h.mismatches:0; if(mm<g.minMM) g.minMM=mm; if(h.wobble&&h.wobble>0) g.wobbleHits++; } const chips=[]; for(const g of groups.values()){ const seedTxt=g.bestSeed?` • ${g.bestSeed}`:''; const mmTxt=Number.isFinite(g.minMM)?` • min mm: ${g.minMM}`:''; const wobTxt=g.wobbleHits?` • wobble hits: ${g.wobbleHits}`:''; chips.push(`<span class="chip">${escapeHTML(g.molecule)}:${escapeHTML(g.id)} • ${g.count} hit(s)${seedTxt}${mmTxt}${wobTxt}</span>`); } if(!chips.length) return ''; return `
    <div style="margin-top:8px;text-align:center;">
      <div style="font-size:12px;color:#475569;margin-bottom:4px;">Seed-site summary (cached from “Seed Sites” scan)</div>
      ${chips.join(' ')}
    </div>
  `; }

// Show canvas as PNG in modal
function showCanvasAsModalPNG(canvas,title,filename){ const data=canvas.toDataURL('image/png'); const img=`<img id="hm-img" src="${data}" alt="Heatmap" style="max-width:100%;height:auto;border:1px solid #e5e7eb;border-radius:8px;"/>`; const tools=`
    <button id="hm-open" class="toolbar-btn">Open in new tab</button>
    <button id="hm-save" class="toolbar-btn">Download PNG</button>
  `; openModal(title,img,tools); const mc=$('modal-content'); if(mc){ const chipsHtml=buildSeedChipSummaryHTML(); if(chipsHtml) appendHTML(mc,chipsHtml); } bindOnce($('hm-open'),'click',()=>{ const w=window.open(data,'_blank'); if(w) w.opener=null; },'hmOpenCanvas'); bindOnce($('hm-save'),'click',()=>{ const a=document.createElement('a'); a.href=data; a.download=filename||'heatmap.png'; document.body.appendChild(a); a.click(); document.body.removeChild(a); },'hmSaveCanvas'); }

// Render simple seed-density strip
function renderSeedDensityFromScan(mirnaSeq,targetId,targetSeq){ const L=targetSeq.length; const density=new Array(L).fill(0); if(Array.isArray(LAST_SEED_HITS)){ LAST_SEED_HITS.filter(h=>h.molecule==='target'&&h.id===targetId).forEach(h=>{ for(let i=Math.max(0,h.start-1);i<Math.min(L,h.end);i++) density[i]+=1; }); } const max=density.reduce((m,v)=>v>m?v:m,1); const norm=density.map(v=>v/max); let strip=`<div style="font-family:ui-monospace, Menlo, Consolas;overflow:auto;border:1px solid #eee;border-radius:8px;padding:8px;"><div style="white-space:nowrap;">`; for(let i=0;i<L;i++){ const color=viridisColor(norm[i]||0,0.85); strip+=`<span title="pos ${i+1} • ${(targetSeq[i]||'')} • ${(norm[i]||0).toFixed(3)}" style="display:inline-block;min-width:10px;padding:2px 0;text-align:center;background:${color};color:#000;border-radius:2px;margin:0 1px;">${escapeHTML(targetSeq[i]||'')}</span>`; } strip+='</div></div>'; const note=Array.isArray(LAST_SEED_HITS)&&LAST_SEED_HITS.length?'':'<br/><small style="color:#666;">Tip: run Seed Sites first for a more informative density.</small>'; return `<div><h4 style="margin:6px 0;">Seed density (client fallback)</h4>${strip}${note}</div>`; }

// =====================================================
// Seed Sites — RANGE-AWARE + tolerant lookup + CSV
// =====================================================
async function handleSeedSitesClick(item){ try{ const allowGU=byQS('#allow-gu')?.checked??true; const maxMM=parseInt(byQS('#max-mm')?.value??'0',10); const mirnaId=item.primary_molecule_id??item.mirna_id; const targetId=item.target_id??''; const compId=item.competitor_id??''; const mirnaSeq=toRNA(lookupTolerant(CURRENT_INPUTS.mirnas,mirnaId)); const tRes=resolveSeqWithAAHandling(targetId,CURRENT_INPUTS.targets); const cRes=compId?resolveSeqWithAAHandling(compId,CURRENT_INPUTS.competitors):{seq:'',converted:false,note:'',mode:''}; const targetSeq=tRes.seq,compSeq=cRes.seq; if(!mirnaSeq||!targetSeq){ const why=(tRes.note||'').trim(); openModal('Seed Sites',formatError('Could not resolve miRNA and/or target sequences for this row. '+(why?` ${escapeHTML(why)}`:''))); return; } const payload={mirna_seq:mirnaSeq,targets:{[targetId]:targetSeq},competitors:compSeq?{[compId]:compSeq}:{},allow_gu:!!allowGU,max_mismatch:Number.isFinite(maxMM)?maxMM:0}; const headers=await getNonceOrKeyHeaders(); const res=await fetch(SEED_SCAN_URL,{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify(payload)}); if(!res.ok){ let msg='Seed scanning failed.'; try{ const j=await res.json(); msg=j.error||msg; }catch(_){} openModal('Seed Sites',formatError(msg)); return; } const data=await res.json(); let hits=Array.isArray(data.hits)?data.hits:[]; if(hits.length===0){ const convNote=(tRes.converted||cRes.converted)?`<div style="margin-top:6px;color:#333;"><small><em>AA→NT conversion was applied prior to scanning.</em></small></div>`:''; openModal('Seed Sites',`<p>No canonical seed matches found under current settings (GU=${allowGU?'on':'off'}, max mismatch=${maxMM}).</p>${convNote}`); return; } const tRange=parseIdRange(targetId); const cRange=compId?parseIdRange(compId):null; hits=hits.map(h=>{ if(h.molecule==='target'&&tRange){ const g=globalCoordForId(targetId,h.start,h.end); return {...h,global_start:g.globalStart,global_end:g.globalEnd}; } if(h.molecule==='competitor'&&cRange){ const g=globalCoordForId(compId,h.start,h.end); return {...h,global_start:g.globalStart,global_end:g.globalEnd}; } return h; }); LAST_SEED_HITS=hits; LAST_SEED_META={mirnaId,targetId,compId}; const showGlobalCols=!!(tRange||cRange); let html=`<div style="margin-bottom:8px;">Found <b>${hits.length}</b> seed-site hit(s). Coordinates are 1-based on the displayed sequence${showGlobalCols?' and global positions are shown when a :start-end range was applied':''}.</div>`; if(tRes.converted||cRes.converted){ const modeTxt=byQS('#aa-nt-mode')?.value||'canonical'; html+=`<div style="margin:6px 0;color:#333;"><small><em>AA→NT conversion applied (${escapeHTML(modeTxt)}) for ${tRes.converted?'target':''}${tRes.converted&&cRes.converted?' & ':''}${cRes.converted?'competitor':''}.</em></small></div>`; } html+=`<table style="width:100%;border-collapse:collapse;"><thead><tr style="text-align:left;border-bottom:1px solid #ddd;"><th>Molecule</th><th>ID</th><th>Start</th><th>End</th>${showGlobalCols?'<th>Global Start</th><th>Global End</th>':''}<th>Seed</th><th>Type</th><th>Wobble</th><th>Mismatches</th><th>Upstream</th></tr></thead><tbody>`; hits.forEach(h=>{ html+=`<tr style="border-bottom:1px solid #f0f0f0;"><td>${escapeHTML(h.molecule)}</td><td>${escapeHTML(h.id)}</td><td>${h.start}</td><td>${h.end}</td>${showGlobalCols?`<td>${h.global_start??''}</td><td>${h.global_end??''}</td>`:''}<td>${h.seed_len}</td><td>${escapeHTML(h.seed_type||'')}</td><td>${h.wobble??0}</td><td>${h.mismatches??0}</td><td>${escapeHTML(h.upstream_base??'')}</td></tr>`; }); html+='</tbody></table>'; const toolbar=`<button id="download-seed-csv" class="btn-action">Download CSV</button>`; openModal('Seed Sites',html,toolbar); const dlBtn=$('download-seed-csv'); if(dlBtn) bindOnce(dlBtn,'click',downloadSeedCSV,'seedCsvOnce'); }catch(err){ openModal('Seed Sites',formatError(err?.message||'Unexpected error during seed scan.')); } }

function downloadSeedCSV(){ if(!Array.isArray(LAST_SEED_HITS)||LAST_SEED_HITS.length===0) return; const showGlobal=LAST_SEED_HITS.some(h=>typeof h.global_start!=='undefined'); const headers=['Molecule','ID','Start','End',...(showGlobal?['Global_Start','Global_End']:[]),'Seed_Len','Seed_Type','Wobble','Mismatches','Upstream_Base']; const rows=[headers.join(',')]; LAST_SEED_HITS.forEach(h=>{ const cells=[h.molecule,h.id,h.start,h.end,...(showGlobal?[h.global_start??'',h.global_end??'']:[]),h.seed_len,h.seed_type||'',h.wobble??0,h.mismatches??0,h.upstream_base??''].map(v=>{ const s=String(v??''); return /[",\n]/.test(s)?`"${s.replace(/"/g,'""')}"`:s; }); rows.push(cells.join(',')); }); const nameBits=[]; if(LAST_SEED_META?.mirnaId) nameBits.push(String(LAST_SEED_META.mirnaId).replace(/[^a-z0-9_\-]+/gi,'_')); if(LAST_SEED_META?.targetId) nameBits.push(String(LAST_SEED_META.targetId).replace(/[^a-z0-9_\-]+/gi,'_')); const fname=`seed_hits_${nameBits.join('__')||'results'}.csv`; const blob=new Blob([rows.join('\n')],{type:'text/csv'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=fname; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }

// =====================================================
// 3D Viewer — Multi-file + Chain/Role + Seed highlights
// =====================================================
async function ensureNGL(){ if(GUARDS.nglLoaded) return true; try{ await new Promise((resolve,reject)=>{ const s=document.createElement('script'); s.src='https://cdn.jsdelivr.net/npm/ngl@latest/dist/ngl.min.js'; s.async=true; s.onload=()=>resolve(); s.onerror=()=>reject(new Error('Failed to load NGL viewer.')); document.head.appendChild(s); }); GUARDS.nglLoaded=true; return true; }catch(_){ return false; } }

async function fetchStructureBlob(kind){ if(!CURRENT_JOB_ID) return null; try{ const headers=await getNonceOrKeyHeaders(); const res=await fetchWithTimeout(STRUCTURE_URL(CURRENT_JOB_ID,kind),{method:'GET',headers},20000); if(!res.ok) return null; const ext=inferExtFromResponse(res); const blob=await res.blob(); return {blob,ext}; }catch(_){ return null; } }

async function open3DCombined(rowItem){ const ok=await ensureNGL(); if(!ok){ openModalText('3D Viewer','Could not load the 3D engine (NGL).'); return; } const title='3D Viewer — Combined (miRNA + Target + Competitor)'; const html=`
  ...
  `; openModal(title,html,tools); const stage=new window.NGL.Stage('ngl-stage',{backgroundColor:'black'}); window.addEventListener('resize',()=>stage.handleResize(),{passive:true}); const compInfos=[],highlightReps=[]; async function loadSource(src,kind){ const comp=await stage.loadFile(src.payload,{ext:src.ext||'pdb'}); const info={comp,id:`${kind}:${src.label}`,label:src.label,kind,chains:[]}; comp.structure.eachChain(cp=>{ const cname=cp.chainname||cp.modelIndex+':'+cp.index; const poly=cp.polymerType||'unknown'; const len=cp.residueCount; const rep=comp.addRepresentation('cartoon',{sele:`:${cname}`,colorScheme:'chainname'}); info.chains.push({name:cname,polymerType:poly,length:len,rep}); }); comp.addRepresentation('ball+stick',{sele:'hetero',multipleBond:true}); compInfos.push(info); } const kinds=['mirna','target','competitor']; for(const k of kinds){ const s=await fetchStructureBlob(k); let extra=collectStructureSources(k,s?.blob||null,s?.ext||'pdb'); extra=filterSourcesForRowByRowItem(extra,k,rowItem); for(const src of extra) await loadSource(src,k); } if(compInfos.length===0){ setHTML($('modal-content'),formatError('No 3D structures were found for miRNA/Target/Competitor. Upload PDB/mmCIF or add PDB IDs in FASTA and re-run.')); return; } stage.autoView(); function rebuildUI(){ const chainList=$('chain-list'); if(!chainList) return; const opts=[]; let html=''; compInfos.forEach((ci,ciIdx)=>{ html+=`<div style="margin:6px 0 2px 0;font-weight:600;">${escapeHTML(ci.kind.toUpperCase())} — ${escapeHTML(ci.label)}</div>`; ci.chains.forEach((ch,chIdx)=>{ html+=`
          ...
          </label>`; opts.push({label:`${ci.kind}:${ch.name}`,ciIdx,chIdx}); }); }); chainList.innerHTML=html; const sM=$('role-mirna'),sT=$('role-target'),sC=$('role-competitor'); [sM,sT,sC].forEach(sel=>{ if(!sel) return; sel.innerHTML='<option value="">— select —</option>'+opts.map(o=>`<option value="${o.ciIdx}:${o.chIdx}">${escapeHTML(o.label)}</option>`).join(''); }); const nucChains=opts.filter(o=>{ const ch=compInfos[o.ciIdx].chains[o.chIdx]; return (ch.polymerType||'').toLowerCase().includes('nuc'); }).sort((a,b)=>{ const la=compInfos[a.ciIdx].chains[a.chIdx].length||0; const lb=compInfos[b.ciIdx].chains[b.chIdx].length||0; return la-lb; }); if(nucChains.length){ const short=nucChains[0]; sM.value=`${short.ciIdx}:${short.chIdx}`; if(nucChains.length>1) sT.value=`${nucChains[nucChains.length-1].ciIdx}:${nucChains[nucChains.length-1].chIdx}`; if(nucChains.length>2) sC.value=`${nucChains[1].ciIdx}:${nucChains[1].chIdx}`; } byQSA('#chain-list input[type="checkbox"]').forEach(cb=>{ cb.addEventListener('change',()=>{ const ci=parseInt(cb.dataset.ci,10); const ch=parseInt(cb.dataset.ch,10); const rep=compInfos[ci].chains[ch].rep; rep.setVisibility(cb.checked); }); }); } rebuildUI(); function rebuildHitSelect(rowScoped=false){ const sel=$('hit-select'); if(!sel) return; let hits=Array.isArray(LAST_SEED_HITS)?[...LAST_SEED_HITS]:[]; if(rowScoped&&rowItem){ hits=hits.filter(h=>(rowItem.target_id&&h.molecule==='target'&&h.id===rowItem.target_id)||(rowItem.competitor_id&&h.molecule==='competitor'&&h.id===rowItem.competitor_id)); } if(!hits.length){ sel.innerHTML='<option value="">— no cached hits —</option>'; return; } sel.innerHTML=hits.map((h,i)=>`<option value="${i}">${h.molecule}:${h.id} • ${h.start}-${h.end}${h.global_start?` (g${h.global_start}-${h.global_end})`:''} • seed${h.seed_len}</option>`).join(''); } rebuildHitSelect(true); bindOnce($('focus-hit'),'click',()=>{ const selBox=$('hit-select'); if(!selBox) return; const idx=parseInt(selBox.value||'-1',10); if(isNaN(idx)||!Array.isArray(LAST_SEED_HITS)||!LAST_SEED_HITS[idx]){ alert('No hit selected. Run Seed Sites first.'); return; } const h=LAST_SEED_HITS[idx]; const which=h.molecule==='competitor'?'competitor':'target'; const selChain=getSelChain(which); if(!selChain){ alert(`Pick a ${which} chain first.`); return; } const offset=parseInt(($('role-offset')?.value||'0'),10)||0; const start=(h.global_start||h.start)+offset; const end=(h.global_end||h.end)+offset; stage.setFocus(selChain.comp.structure.getView(new window.NGL.Selection(`:${selChain.name} and resno ${start}-${end}`))); },'focusHitOnce_combined'); function getSelChain(which){ const map={mirna:'role-mirna',target:'role-target',competitor:'role-competitor'}; const el=$(map[which]); if(!el||!el.value) return null; const [ciIdx,chIdx]=el.value.split(':').map(x=>parseInt(x,10)); const ci=compInfos[ciIdx]; if(!ci) return null; const ch=ci.chains[chIdx]; if(!ch) return null; return {ciIdx,chIdx,comp:ci.comp,name:ch.name,rep:ch.rep}; } function focusChain(which){ const sel=getSelChain(which); if(!sel) return; stage.setFocus(sel.comp.structure.getView(new window.NGL.Selection(`:${sel.name}`))); } function clearHighlights(){ while(highlightReps.length){ const r=highlightReps.pop(); try{ r.component.removeRepresentation(r.rep); }catch(_){} } } function highlightSeedSites(){ clearHighlights(); const targetSel=getSelChain('target'); if(!targetSel){ alert('Pick a Target chain first.'); return; } const offset=parseInt(($('role-offset')?.value||'0'),10)||0; const hits=Array.isArray(LAST_SEED_HITS)?LAST_SEED_HITS.filter(h=>h.molecule==='target'&&(!LAST_SEED_META||!LAST_SEED_META.targetId||h.id===LAST_SEED_META.targetId)):[]; if(!hits.length){ alert('No cached seed hits for this row. Run “Seed Sites” first.'); return; } hits.forEach(h=>{ const start=(h.global_start||h.start)+offset; const end=(h.global_end||h.end)+offset; const rep=targetSel.comp.addRepresentation('spacefill',{sele:`:${targetSel.name} and resno ${start}-${end}`}); highlightReps.push({component:targetSel.comp,rep}); }); stage.autoView(); } bindOnce($('role-focus-m'),'click',()=>focusChain('mirna'),'rfm'); bindOnce($('role-focus-t'),'click',()=>focusChain('target'),'rft'); bindOnce($('role-focus-c'),'click',()=>focusChain('competitor'),'rfc'); bindOnce($('seed-highlight'),'click',highlightSeedSites,'seedhl'); bindOnce($('clear-highlights'),'click',clearHighlights,'seedcl'); bindOnce($('ngl-center'),'click',()=>stage.autoView(),'nglcenter2'); bindOnce($('ngl-snap'),'click',async()=>{ try{ const img=await stage.makeImage({factor:2,antialias:true,trim:false,transparent:false}); if(img instanceof Blob){ const url=URL.createObjectURL(img); const a=document.createElement('a'); a.href=url; a.download='structure_combined.png'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); return; } if(img&&typeof img.toDataURL==='function'){ const url=img.toDataURL('image/png'); const a=document.createElement('a'); a.href=url; a.download='structure_combined.png'; if(typeof a.download==='string'){ document.body.appendChild(a); a.click(); document.body.removeChild(a); }else{ const w=window.open(url,'_blank'); if(w) w.opener=null; } return; } if(stage.viewer&&typeof stage.viewer.makeImage==='function'){ stage.viewer.makeImage({factor:2,antialias:true,trim:false,transparent:false}).then(blob=>{ const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='structure_combined.png'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }); return; } alert('Snapshot failed: unsupported return type.'); }catch(e){ alert('Snapshot failed.'); } },'nglsnap2'); }

async function open3DOrExplain(anyId,kind,rowItem=null){ if(!CURRENT_JOB_ID){ openModalText('3D Viewer','Run a prediction first.'); return; } let primaryBlob=null,primaryExt='pdb'; try{ const headers=await getNonceOrKeyHeaders(); const res=await fetchWithTimeout(STRUCTURE_URL(CURRENT_JOB_ID,kind),{method:'GET',headers},20000); if(res.ok){ primaryExt=inferExtFromResponse(res); primaryBlob=await res.blob(); } }catch(_){ } await open3DStageManager(kind,anyId,{blob:primaryBlob,ext:primaryExt},rowItem); }

async function open3DStageManager(kind,anyId,primary,rowItem){ const ok=await ensureNGL(); if(!ok){ openModalText('3D Viewer','Could not load the 3D engine (NGL).'); return; } const prettyKind=kind==='mirna'?'miRNA':(kind==='target'?'Target':'Competitor'); const title=`3D Viewer — ${prettyKind}${anyId?' • '+anyId:''}`; const html=`
    <div style="display:grid;grid-template-columns:280px 1fr;gap:10px;max-height:86vh;">
      <div id="ngl-side" style="overflow:auto;border:1px solid #e5e7eb;border-radius:10px;padding:10px;">
        <div style="font-weight:700;margin-bottom:6px;">Chains & Roles</div>
        <div id="chain-list"></div>
  ...
  `; const tools=`
    <button id="ngl-center" class="toolbar-btn">Center</button>
    <button id="ngl-snap" class="toolbar-btn">Snapshot PNG</button>
  `; openModal(title,html,tools); const stage=new window.NGL.Stage('ngl-stage',{backgroundColor:'black'}); window.addEventListener('resize',()=>stage.handleResize(),{passive:true}); let sources=collectStructureSources(kind,primary?.blob||null,primary?.ext||'pdb'); sources=filterSourcesForRowByRowItem(sources,kind,rowItem); if(sources.length===0){ setHTML($('modal-content'),formatError(`No 3D structures found for ${prettyKind}. Upload PDB/mmCIF or add a PDB ID in FASTA and re-run.`)); return; } const compInfos=[],highlightReps=[]; async function loadSource(src){ const comp=await stage.loadFile(src.payload,{ext:src.ext||'pdb'}); const info={comp,id:`${kind}:${src.label}`,label:src.label,kind,chains:[]}; comp.structure.eachChain(cp=>{ const cname=cp.chainname||cp.modelIndex+':'+cp.index; const poly=cp.polymerType||'unknown'; const len=cp.residueCount; const rep=comp.addRepresentation('cartoon',{sele:`:${cname}`,colorScheme:'chainname'}); info.chains.push({name:cname,polymerType:poly,length:len,rep}); }); comp.addRepresentation('ball+stick',{sele:'hetero',multipleBond:true}); compInfos.push(info); } for(const src of sources) await loadSource(src); stage.autoView(); function rebuildUI(){ const chainList=$('chain-list'); if(!chainList) return; const opts=[]; let html=''; compInfos.forEach((ci,ciIdx)=>{ html+=`<div style="margin:6px 0 2px 0;font-weight:600;">${escapeHTML(ci.label)}</div>`; ci.chains.forEach((ch,chIdx)=>{ html+=`
          <label style="display:flex;align-items:center;gap:6px;margin:2px 0;">
            <input type="checkbox" data-ci="${ciIdx}" data-ch="${chIdx}" checked />
            <span style="font-family:ui-monospace;">${escapeHTML(ch.name)}</span>
            <small class="chip">${escapeHTML(ch.polymerType||'')}</small>
            <small class="chip">${ch.length||''}</small>
          </label>`; opts.push({label:`${ci.kind||prettyKind}:${ch.name}`,ciIdx,chIdx}); }); }); chainList.innerHTML=html; const sM=$('role-mirna'),sT=$('role-target'),sC=$('role-competitor'); [sM,sT,sC].forEach(sel=>{ if(!sel) return; sel.innerHTML='<option value="">— select —</option>'+opts.map(o=>`<option value="${o.ciIdx}:${o.chIdx}">${escapeHTML(o.label)}</option>`).join(''); }); const nucChains=opts.filter(o=>{ const ch=compInfos[o.ciIdx].chains[o.chIdx]; return (ch.polymerType||'').toLowerCase().includes('nuc'); }).sort((a,b)=>{ const la=compInfos[a.ciIdx].chains[a.chIdx].length||0; const lb=compInfos[b.ciIdx].chains[b.chIdx].length||0; return la-lb; }); if(nucChains.length){ sM.value=`${nucChains[0].ciIdx}:${nucChains[0].chIdx}`; if(nucChains.length>1) sT.value=`${nucChains[nucChains.length-1].ciIdx}:${nucChains[nucChains.length-1].chIdx}`; if(nucChains.length>2) sC.value=`${nucChains[1].ciIdx}:${nucChains[1].chIdx}`; } byQSA('#chain-list input[type="checkbox"]').forEach(cb=>{ cb.addEventListener('change',()=>{ const ci=parseInt(cb.dataset.ci,10); const ch=parseInt(cb.dataset.ch,10); const rep=compInfos[ci].chains[ch].rep; rep.setVisibility(cb.checked); }); }); } rebuildUI(); function rebuildHitSelect(rowScoped=false){ const sel=$('hit-select'); if(!sel) return; let hits=Array.isArray(LAST_SEED_HITS)?[...LAST_SEED_HITS]:[]; if(rowScoped&&rowItem){ hits=hits.filter(h=>(rowItem.target_id&&h.molecule==='target'&&h.id===rowItem.target_id)||(rowItem.competitor_id&&h.molecule==='competitor'&&h.id===rowItem.competitor_id)); } if(!hits.length){ sel.innerHTML='<option value="">— no cached hits —</option>'; return; } sel.innerHTML=hits.map((h,i)=>`<option value="${i}">${h.molecule}:${h.id} • ${h.start}-${h.end}${h.global_start?` (g${h.global_start}-${h.global_end})`:''} • seed${h.seed_len}</option>`).join(''); } rebuildHitSelect(!!rowItem); bindOnce($('focus-hit'),'click',()=>{ const selBox=$('hit-select'); if(!selBox) return; const idx=parseInt(selBox.value||'-1',10); if(isNaN(idx)||!Array.isArray(LAST_SEED_HITS)||!LAST_SEED_HITS[idx]){ alert('No hit selected. Run Seed Sites first.'); return; } const h=LAST_SEED_HITS[idx]; const which=h.molecule==='competitor'?'competitor':'target'; const selChain=getSelChain(which); if(!selChain){ alert(`Pick a ${which} chain first.`); return; } const offset=parseInt(($('role-offset')?.value||'0'),10)||0; const start=(h.global_start||h.start)+offset; const end=(h.global_end||h.end)+offset; stage.setFocus(selChain.comp.structure.getView(new window.NGL.Selection(`:${selChain.name} and resno ${start}-${end}`))); },'focusHitOnce_single'); function getSelChain(which){ const map={mirna:'role-mirna',target:'role-target',competitor:'role-competitor'}; const el=$(map[which]); if(!el||!el.value) return null; const [ciIdx,chIdx]=el.value.split(':').map(x=>parseInt(x,10)); const ci=compInfos[ciIdx]; if(!ci) return null; const ch=ci.chains[chIdx]; if(!ch) return null; return {ciIdx,chIdx,comp:ci.comp,name:ch.name,rep:ch.rep}; } function focusChain(which){ const sel=getSelChain(which); if(!sel) return; stage.setFocus(sel.comp.structure.getView(new window.NGL.Selection(`:${sel.name}`))); } function clearHighlights(){ while(highlightReps.length){ const r=highlightReps.pop(); try{ r.component.removeRepresentation(r.rep); }catch(_){} } } function highlightSeedSites(){ clearHighlights(); const targetSel=getSelChain('target'); if(!targetSel){ alert('Pick a Target chain first.'); return; } const offset=parseInt(($('role-offset')?.value||'0'),10)||0; let hits=Array.isArray(LAST_SEED_HITS)?LAST_SEED_HITS.filter(h=>h.molecule==='target'):[]; if(rowItem?.target_id) hits=hits.filter(h=>h.id===rowItem.target_id); if(!hits.length){ alert('No cached seed hits for this row. Run “Seed Sites” first.'); return; } hits.forEach(h=>{ const start=(h.global_start||h.start)+offset; const end=(h.global_end||h.end)+offset; const rep=targetSel.comp.addRepresentation('spacefill',{sele:`:${targetSel.name} and resno ${start}-${end}`}); highlightReps.push({component:targetSel.comp,rep}); }); stage.autoView(); } bindOnce($('role-focus-m'),'click',()=>focusChain('mirna'),'rfm_s'); bindOnce($('role-focus-t'),'click',()=>focusChain('target'),'rft_s'); bindOnce($('role-focus-c'),'click',()=>focusChain('competitor'),'rfc_s'); bindOnce($('seed-highlight'),'click',highlightSeedSites,'seedhl_s'); bindOnce($('clear-highlights'),'click',clearHighlights,'seedcl_s'); bindOnce($('ngl-center'),'click',()=>stage.autoView(),'nglcenter_s'); bindOnce($('ngl-snap'),'click',async()=>{ try{ const img=await stage.makeImage({factor:2,antialias:true,trim:false,transparent:false}); const fname=`structure_${kind}.png`; if(img instanceof Blob){ const url=URL.createObjectURL(img); const a=document.createElement('a'); a.href=url; a.download=fname; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); return; } if(img&&typeof img.toDataURL==='function'){ const url=img.toDataURL('image/png'); const a=document.createElement('a'); a.href=url; a.download=fname; if(typeof a.download==='string'){ document.body.appendChild(a); a.click(); document.body.removeChild(a); }else{ const w=window.open(url,'_blank'); if(w) w.opener=null; } return; } if(stage.viewer&&typeof stage.viewer.makeImage==='function'){ stage.viewer.makeImage({factor:2,antialias:true,trim:false,transparent:false}).then(blob=>{ const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=fname; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }); return; } alert('Snapshot failed: unsupported return type.'); }catch(e){ alert('Snapshot failed.'); } },'nglsnap_s'); }

// Legacy minimal viewer
async function open3DViewer(kind){ if(!CURRENT_JOB_ID){ openModalText('3D Viewer','Run a prediction first.'); return; } if(!['target','competitor','mirna'].includes(kind)){ openModalText('3D Viewer','Invalid molecule kind.'); return; } const ok=await ensureNGL(); if(!ok){ openModalText('3D Viewer','Could not load 3D engine.'); return; } try{ const headers=await getNonceOrKeyHeaders(); const res=await fetch(STRUCTURE_URL(CURRENT_JOB_ID,kind),{method:'GET',headers}); if(!res.ok){ openModalText('3D Viewer','No 3D structure available (maybe not uploaded or expired).'); return; } const blob=await res.blob(); await open3DStageManager(kind,'',{blob,ext:inferExtFromResponse(res)},null); }catch(err){ openModalText('3D Viewer',err?.message||'3D viewer error.'); } }

// =====================================================
// Tabs / helpers
// =====================================================
function openTab(element,tabId){ const targetCard=$(tabId); if(!targetCard) return; byQSA('.card').forEach(card=>card.classList.remove('active')); byQSA('.tab-btn').forEach(btn=>btn.classList.remove('active')); targetCard.classList.add('active'); if(element&&element.classList) element.classList.add('active'); requestAnimationFrame(()=>{ requestAnimationFrame(()=>{ scrollToCardTop(tabId); }); }); }

function makeTableSortable(tableId){ const table=document.getElementById(tableId); if(!table) return; table.querySelectorAll('th').forEach((header,idx)=>{ header.style.cursor='pointer'; header.addEventListener('click',()=>{ const rows=Array.from(table.querySelectorAll('tbody tr')); const asc=header.classList.toggle('asc'); rows.sort((a,b)=>{ const aText=a.children[idx].textContent.trim(); const bText=b.children[idx].textContent.trim(); const na=parseFloat(aText),nb=parseFloat(bText); if(!Number.isNaN(na)&&!Number.isNaN(nb)) return asc?na-nb:nb-na; return asc?aText.localeCompare(bText):bText.localeCompare(aText); }); const tbody=table.querySelector('tbody'); rows.forEach(row=>tbody.appendChild(row)); }); }); }

function getTabsBarEl(){ return document.querySelector('#tabs-bar, .tabs-bar, .tabbar, .tabs, .sticky-tabs, .tab-bar-sticky'); }
function getTabsBarHeight(){ const el=getTabsBarEl(); return el?Math.round(el.getBoundingClientRect().height)||0:0; }

function ensureTabsAnchor(){ const tabsBar=getTabsBarEl(); if(!tabsBar) return; if(document.getElementById('tabs-anchor')) return; const a=document.createElement('div'); a.id='tabs-anchor'; a.style.cssText='height:0;margin:0;padding:0;scroll-margin-top:calc(var(--sticky-offset-main) + var(--sticky-gap));'; tabsBar.parentNode.insertBefore(a,tabsBar); }

function scrollTabsToTop(){ ensureTabsAnchor(); const a=document.getElementById('tabs-anchor'); if(!a) return; a.scrollIntoView({block:'start',behavior:'smooth'}); requestAnimationFrame(()=>a.scrollIntoView({block:'start',behavior:'auto'})); }

function gotoResultsTab(){ const resultsTabButton=Array.from(document.querySelectorAll('button.tab-btn')).find(b=>/results/i.test(b.textContent||'')); if(resultsTabButton){ openTab(resultsTabButton,'results-tab'); scrollTabsToTop(); } }

function wireTabButtonsOnce(){ if(GUARDS.tabWiringDone) return; const tabs=byQSA('.tab-btn'); const loader=$('loader'); tabs.forEach(btn=>{ bindOnce(btn,'click',ev=>{ ev.preventDefault(); const targetId=targetIdFromButton(btn); if(targetId){ openTab(btn,targetId); ensureStickyGapForTabs(); } const name=(btn.textContent||'').toLowerCase(); if(name.includes('inputs')){ if(loader){ text(loader,'Please input your sequences to start a prediction.'); show(loader); } } if(name.includes('results')){ const rc=$('results-container'); if(rc&&!rc.innerHTML.trim()) setHTML(rc,formatInfo('Results will appear here after you run a prediction.')); } if(name.includes('workflow')){ if(loader) hide(loader); } },'tabClickSnap'); }); GUARDS.tabWiringDone=true; }

function getStickySum(includeTabs=true){ const root=getComputedStyle(document.documentElement); const off=parseInt(root.getPropertyValue('--sticky-offset-main')||'96',10)||96; const gap=parseInt(root.getPropertyValue('--sticky-gap')||'12',10)||12; const tabs=includeTabs?getTabsBarHeight():0; return off+gap+tabs; }

// =====================================================
// Modal (singleton)
// =====================================================
function ensureModal(){ if(GUARDS.modalInjected) return; const body=document.body; ensureSingleton('analysis-modal',`
    ...
    `,body); const modal=$('analysis-modal'); const closeBtn=$('modal-close'); const overlay=modal?.querySelector('[data-overlay]'); if(closeBtn) bindOnce(closeBtn,'click',closeModal,'mclose'); if(overlay) bindOnce(overlay,'click',closeModal,'moverlay'); GUARDS.modalInjected=true; }
function openModal(title,html,toolbarHTML=''){ ensureModal(); const modal=$('analysis-modal'); if(!modal) return; text($('modal-title'),title||'Analysis'); setHTML($('modal-content'),html||''); setHTML($('modal-tools'),toolbarHTML||''); modal.style.display='flex'; }
function closeModal(){ const modal=$('analysis-modal'); if(modal) modal.style.display='none'; }

// =====================================================
// Sticky header offset sync
// =====================================================
function syncStickyOffset(){ const header=document.querySelector('header.site-header, header#site-header, .site-header, header#main-header'); const h=header?Math.max(64,Math.round(header.getBoundingClientRect().height)):96; document.documentElement.style.setProperty('--sticky-offset-main',`${h}px`); }

// Guard for missing elements
(function guardMissingElements(){ const form=$('prediction-form'); if(!form) console.warn('Prediction form not found on page.'); })();
